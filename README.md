...
1. **`/design`** (ומקבילותיו `/design/poster`, `/design/tote`, `/design/canvas`,
   `/design/mug`) — הלקוח בוחר סוג מוצר / צבע / מידה, מעלה תמונה, וממקם אותה
   על קנבס Konva (גרירה להזזה, ידיות בפינות להגדלה/הקטנה/סיבוב). הקבצים
   (התמונה המקורית, תמונת ה-mockup המורכבת, וקובץ ההדפסה) מועלים ישירות
   מהדפדפן ל-**Vercel Blob** דרך `@vercel/blob/client` (`/api/blob-upload`
   רק מנפיק טוקן — הבייטים עצמם לא עוברים דרך שרת שלנו, כדי לא להיתקל
   במגבלת גודל body של serverless function עם תמונה אמיתית מהטלפון).
   URL ציבורי עם סיומת אקראית — ככה Gelato יכול למשוך את הקובץ ישירות.
