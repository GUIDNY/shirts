...
export interface NewOrder {
  ...
  canvas_orientation: string | null;
  quantity: number;
  image_url: string;
  mockup_url: string;
...
export async function insertOrder(order: NewOrder): Promise<OrderRecord> {
  const rows = (await sql()`
    insert into orders (
      ...
      product_category, product_type, size, color, poster_paper, poster_orientation, tote_color, canvas_orientation,
      ...
      ${order.poster_paper}, ${order.poster_orientation}, ${order.tote_color}, ${order.canvas_orientation}, ${order.quantity},
